...
import { gradients, photos, videoFormats, getGradientCSS } from '../lib/backgrounds';
...
export default function RecitationStudio({ surah, verses, onBack }: Props) {
  ...
  const [formatIdx, setFormatIdx] = useState(1); // default 9:16
  ...
  const goNext = () => { audioRef.current?.pause(); if (currentIndex < verses.length - 1) setCurrentIndex(prev => prev + 1); };
  const goPrev = () => { audioRef.current?.pause(); if (currentIndex > 0) setCurrentIndex(prev => prev - 1); };
  ...
  const handleFileImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setSelectedBg({ id: 'imported', name: 'Imported', type: 'photo', url: dataUrl, thumb: dataUrl } as PhotoBackground);
      // Also preload it as an image element
      const img = new Image();
  ...
  const drawFrame = (ctx: CanvasRenderingContext2D, verse: Verse, w: number, h: number, bgImage?: HTMLImageElement | null) => {
    // Background
    if (bgImage) {
    ...
    ctx.font = '400 ' + Math.round(14 * scale) + 'px Inter, system-ui, sans-serif';
    ctx.direction = 'ltr';
    ctx.textAlign = 'center';
    ctx.fillText('Quran SM Download', w / 2, h * 0.95);
  };
  ...
  const downloadImage = useCallback(async () => {
    ...
    setGenProgress(10);
    try {
      // Wait for photo to load if needed
      let bgImg = getBgImage();
      if (selectedBg.type === 'photo' && !bgImg) {
      ...
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = surah.englishName + '-verse-' + currentVerse.numberInSurah + '-' + format.label + '.png';
        a.click();
        URL.revokeObjectURL(url);
        setGenProgress(100);
        setTimeout(() => { setIsGenerating(false); setGenType(null); }, 400);
      }, 'image/png');
    } catch {
      alert('Failed to generate image. Try a gradient background if photo fails.');
      setIsGenerating(false);
      setGenType(null);
    }
  }, [currentVerse, selectedBg, format, showTranslation, arabicSize, translationSize, overlayOpacity, photoCache]);
    ...
    try {
      ...
      for (let i = 0; i < verses.length; i++) {
        ...
        try {
          ...
          audio.volume = 0; // mute playback during recording
          await audio.play().catch(() => {});
          // Wait for natural end or timeout
          await new Promise<void>(resolve => {
            audio.onended = () => resolve();
            audio.onerror = () => resolve();
            setTimeout(() => { audio.pause(); resolve(); }, 15000);
          });
        } catch {
      ...
      const a = document.createElement('a');
      a.href = url2;
      a.download = surah.englishName + '-recitation-' + format.label + '.webm';
      a.click();
      URL.revokeObjectURL(url2);
      setGenProgress(100);
    } catch {
      alert('Video generation failed. Try Chrome on desktop.');
    } finally {
      setTimeout(() => { setIsGenerating(false); setGenType(null); }, 500);
    }
  }, [verses, selectedBg, reciter, surah, showTranslation, format, arabicSize, translationSize, overlayOpacity, photoCache]);

  // Get preview background CSS
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)' }}>
          <div className="text-center p-8 rounded-2xl max-w-sm w-full mx-4" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>
              {genType === 'video' ? 'Recording all verses. This may take a while.' : 'Processing...'}
            </p>
      ...
      <div className="flex flex-col lg:flex-row" style={{ height: 'calc(100vh - 56px)' }}>
        ...
        <div className={
          ...
          <div className="p-4 space-y-5">
            {/* Video Format */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider mb-3 block" style={{ color: 'var(--text-secondary)' }}>Video Format</label>
            ...
            <div>
              ...
              <input type="range" min="40" max="120" value={arabicSize} onChange={e => setArabicSize(Number(e.target.value))} className="w-full accent-current" style={{ accentColor: 'var(--accent)' }} />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setShowTranslation(!showTranslation)} className="w-11 h-6 rounded-full transition-all relative" style={{ background: showTranslation ? 'var(--accent)' : 'var(--bg-card)', border: '1px solid var(--border)' }}>
                  <div className="w-4.5 h-4.5 rounded-full bg-white absolute top-0.5 transition-all" style={{ width: 18, height: 18, left: showTranslation ? '22px' : '3px' }} />
                </button>
              </div>
              {showTranslation && (
            ...
            <div>
              ...
              {bgTab === 'import' && (
                <div>
                  ...
                  {importedImage && (
                    <button onClick={() => setSelectedBg({ id: 'imported', name: 'Imported', type: 'photo', url: importedImage, thumb: importedImage } as PhotoBackground)} className="mt-2 w-full rounded-xl overflow-hidden" style={{ outline: selectedBg.id === 'imported' ? '2px solid var(--accent)' : '2px solid transparent', outlineOffset: '2px', height: 60 }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex items-center justify-center p-4 sm:p-6 overflow-y-auto" style={{ background: 'var(--bg-primary)' }}>
            <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ aspectRatio: format.ratio, maxHeight: '100%', maxWidth: '100%', width: format.id === 'portrait' ? 320 : format.id === 'square' ? 400 : format.id === 'social' ? 360 : 560 }}>
              <div className="w-full h-full flex flex-col items-center justify-center p-6 sm:p-10 relative" style={getPreviewBgCSS()}>
                ...
                <p className="absolute bottom-3 text-white/20 text-[10px] tracking-wider">Quran SM Download</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t" style={{ borderColor: 'var(--border)' }}>
            <div className="flex overflow-x-auto gap-1 p-2 sm:p-3 custom-scrollbar">
              {verses.map((v, i) => (
                <button key={v.numberInSurah} onClick={() => { setCurrentIndex(i); if (isPlaying) { audioRef.current?.pause(); setIsPlaying(true); } }} className="flex-shrink-0 w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center text-sm font-bold transition-all hover:scale-110" style={{ background: i === currentIndex ? 'var(--accent)' : 'var(--bg-card)', color: i === currentIndex ? 'white' : 'var(--text-secondary)', boxShadow: i === currentIndex ? '0 4px 20px var(--accent-light)' : 'none' }}>
                  {v.numberInSurah}
                </button>
              ))}
            </div>
